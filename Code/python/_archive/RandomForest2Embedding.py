...
def generate_synthetic_features(X, method='bootstrap', random_state=12345):
    ...
    for column in range(n_features):
        if method == 'bootstrap':
            synth_X[:, column] = bootstrap_sample_column(X[:, column],
                    random_state=random_state)
        elif method == 'uniform':
            synth_X[:, column] = uniform_sample_column(X[:, column],
                    random_state=random_state)
        else:
...
def generate_discriminative_dataset(X, method='bootstrap', random_state=1234):
    ...
    synth_X = generate_synthetic_features(X, method=method,
            random_state=random_state)
...
def calcul_embedding(nodes, n_components=2):
    ...
    """
    projecteur = manifold.TSNE(n_components=n_components, random_state=12345,
                               metric='hamming')
...
class RandomForest2Embedding(BaseForest):
    ...
    def __init__(self, n_estimators=10, criterion='gini', max_depth=5,
                 min_samples_split=2, min_samples_leaf=1,
                 min_weight_fraction_leaf=0., max_features='auto',
                 max_leaf_nodes=None, bootstrap=True, method='bootstrap',
                 n_jobs=-1, random_state=None, verbose=0, warm_start=False):
        super(RandomForest2Embedding, self).__init__(
                base_estimator=DecisionTreeClassifier(),
                n_estimators=n_estimators, estimator_params=(
                "criterion", "max_depth", "min_samples_split",
                "min_samples_leaf", "min_weight_fraction_leaf", "max_features",
                "max_leaf_nodes", "random_state"), bootstrap=bootstrap,
                oob_score=False, n_jobs=n_jobs, random_state=random_state,
