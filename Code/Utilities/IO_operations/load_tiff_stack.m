function varargout = load_tiff_stack(filename, img_first, img_last)
...
if (tiff_id ~= 42)
    error('tiffReader:cannotRead', 'This is not a TIFF file (missing 42).');
end
...
while (ifd_pos ~= 0)
    ...
    if isfield( TIF, 'MM_stack')
        error('Impossible to read metamorph stacks')
%         stack = [];
...
%         break
        ...
        if img_skip + 1 >= img_first
            ...
            try
                stack_in_raw(:, :, img_read) = IMG.data;
            catch
...
if nargout > 0
    varargout{1} = stack_in_raw;
end
if nargout == 2
    TIF = rmfield(TIF, 'file');
    varargout{2} = TIF;
end
