function Rpath = find_path_of_R()
    ...
    for i = 1:length(n)
        ...
        D = dir(os.path.join(programPath, 'R'));
        if ~isempty(D)
            A = {D.name};
            B = find(cell2mat(cellfun(@(s) ~isempty(strfind(s,'R-')),A,'uniformoutput',0)),1); %#ok<STREMP>
            if ~isempty(B)
                Rpath{end + 1} = os.path.join(programPath, 'R', A{B}, 'bin');
            end
        end
    end
    if isempty(Rpath)
        ...
        if length(Rpath) > 1
            % Take latest version
            Rpath_parts = cellfun(@(x) strsplit(x, filesep()), Rpath, 'UniformOutput',false);
            Rpath_parts = natsort(cellfun(@(x) x{end-1}, Rpath_parts, 'UniformOutput',false));
            Rpath = Rpath_parts{end};
        else
            Rpath = Rpath{1};
        end
    end
