...
if ~isempty(varargin) && isfield(varargin{1}, 'lineThickness')
    ...
    lineThickness = 0.5; % default thickness is 0.5, like in normal axis
end
...
for i = 1:length(tickLocations)
    if ~ismember(tickLocations(i),[start, fin]) % these have already been plotted
        ...
        set(h,'color', color, 'lineWidth', lineThickness, 'clipping','off');
            outParams.ticks(end+1,1) = h;
    end
end

% PLOT NUMERICAL LABELS (presumably on the ticks)
outParams.ticksLabel = [];
...
for i = 1:length(tickLabelLocations)
    ...
    set(h,'HorizontalA', NalignH, 'VerticalA', NalignV, 'fontsize', fontSize, 'color', color, 'clipping','off');
        outParams.ticksLabel(end+1,1) = h;
end
...
set(h,'HorizontalA', LalignH, 'VerticalA', LalignV, 'fontsize', fontSize+1, 'color', color, 'clipping','off');
if axisOrientation == 'v', set(h,'rotation',90); end
    outParams.axisLabel = h;
% DONE PLOTTING
