...
if strcmpi(ctype,'qual')
    if ncol > size(colorbrewer.(ctype).(cname),1)
        cbrew_init = interpolate_cbrewer (colorbrewer.(ctype).(cname), interp_method, ncol);
    else
        cbrew_init = colorbrewer.(ctype).(cname)(1:ncol,:);
    end
    colormap = cbrew_init./255;
    return
end
