...
theta        = atan( (y2-y1)/den ) + pi*(x2<x1) - pi/2;      % angle of arrow
cs          = cos(theta);                                    % rotation matrix
ss          = sin(theta);
R           = [cs -ss;ss cs];
...
% =============================================
line_length2 = line_length * .9;
alpha = atan2(y2-y1,x2-x1);
y2 = y1 + line_length2 * sin(alpha);
x2 = x1 + line_length2 * cos(alpha);
